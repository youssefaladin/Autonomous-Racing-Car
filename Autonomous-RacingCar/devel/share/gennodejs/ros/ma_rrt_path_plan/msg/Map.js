...
// (in-package ma_rrt_path_plan.msg)
...
class Map {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.cones[i] = geometry_msgs.msg.Point.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.cones !== undefined) {
      ...
      for (let i = 0; i < resolved.cones.length; ++i) {
        resolved.cones[i] = geometry_msgs.msg.Point.Resolve(msg.cones[i]);
      }
    }
    else {
      resolved.cones = []
