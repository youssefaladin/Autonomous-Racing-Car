...
from ma_rrt_path_plan.msg import Waypoint
...
class MaRRTPathPlanNode:
    ...
    def __init__(self):
        ...
        rospy.loginfo("Parameter ~publishWaypoints: %s", self.shouldPublishWaypoints)
        rospy.loginfo("Parameter ~publishPredefined: %s", self.shouldPublishPredefined)
        ...
        rospy.Subscriber("/ground_truth/state_raw", Odometry, self.odometryCallback)
        #rospy.Subscriber("/ground_truth/state_raw", Odometry, self.odometryCallback)
    ...
    def odometryCallback(self, odometry):
        # For local-frame operation, ignore the global odometry values and treat the car as at (0,0) with zero yaw.
        self.carPosX = 0.0
        self.carPosY = 0.0
        self.carPosYaw = 0.0
        rospy.loginfo("odometryCallback (local): pos=(%.2f, %.2f), yaw=%.2f", 
    ...
    def mapCallback(self, map):
        self.map = map
        rospy.loginfo("mapCallback in planner: Received map with %d cones", len(map.cones))
        for i, cone in enumerate(map.cones):
    ...
    def sampleTree(self):
        ...
        rospy.loginfo("Found %d front cones", len(frontCones))
        ...
        rospy.loginfo("RRT planning finished: nodeList size=%d, leafNodes size=%d", len(nodeList), len(leafNodes))
        ...
        if bestBranch:
            rospy.loginfo("Best branch found, proceeding to filtering and waypoint extraction")

            filteredBestBranch = self.getFilteredBestBranch(bestBranch)
            # print "filteredBestBranch", filteredBestBranch

            if filteredBestBranch:
                rospy.loginfo("Filtered branch obtained. Computing Delaunay edges.")

                # Delaunay
                # delaunayStartTime = time.time()
                delaunayEdges = self.getDelaunayEdges(frontCones)
                rospy.loginfo("Found %d Delaunay edges", len(delaunayEdges))
    ...
    def mergeWaypoints(self, newWaypoints):
        ...
        maxDistToSaveWaypoints = 2.5
        maxWaypointAmountToSave = 2
        waypointsDistTollerance = 0.7
    ...
    def publishWaypoints(self, newWaypoints = None):
        ...
        waypointsArray.header.frame_id = "base_link"
        ...
        rospy.loginfo("publishWaypoints(): Publishing %d total waypoints",
                  len(waypointsArray.waypoints))
        ...
        rospy.loginfo("publishWaypoints(): Publishing %d total waypoints",
                  len(waypointsArray.waypoints))
        for wp in waypointsArray.waypoints:
                rospy.logdebug("Waypoint ID=%d, x=%.2f, y=%.2f", wp.id, wp.x, wp.y)
    ...
    def publishWaypointsVisuals(self, newWaypoints = None):
        ...
        if newWaypoints is not None:
            ...
            newWaypointsMarker.header.frame_id = "base_link"
    ...
    def getFrontConeObstacles(self, map, frontDist):
        ...
        for cone in map.cones:
            # Compute the cross product result to determine side
            side_value = headingVectorOrt[0] * (cone.y - carPosBehindPoint[1]) - \
                        headingVectorOrt[1] * (cone.x - carPosBehindPoint[0])
            # Compute squared distance
            distSq = (cone.x - self.carPosX) ** 2 + (cone.y - self.carPosY) ** 2
            rospy.logdebug("Cone at (%.2f, %.2f): side_value=%.2f, distSq=%.2f", cone.x, cone.y, side_value, distSq)
            if side_value < 0 and distSq < frontDistSq:
                    frontConeList.append(cone)
        rospy.loginfo("getFrontConeObstacles: Found %d front cones (frontDist=%.2f)", len(frontConeList), frontDist)
