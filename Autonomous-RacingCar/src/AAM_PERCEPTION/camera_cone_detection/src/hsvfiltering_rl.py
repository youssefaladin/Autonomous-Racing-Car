...
class hsvfilter():
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber("/zed2i/zed_node/left/image_rect_color",Image,self.callbackimage)
        #self.image_sub2 = rospy.Subscriber("/zed/left/image_rect_color",Image,self.callbackimage2)
        self.pointcloud_sub = rospy.Subscriber("/zed2i/zed_node/point_cloud/cloud_registered",PointCloud2,self.pointcloudcall)
    ...
    def filter(self):
        ...
        if self.keys[0] == 1 and self.keys[1] == 1 and self.keys[2]==1:
            ...
            header.frame_id = self.my_pcl.header.frame_id
              # Replace with the appropriate frame ID
                      ...
                      for m in range (miny,maxy):
                            ...
                            if s<0.3 or v<0.3:
                                ...
                                if math.isnan(x): 
                                   ...
                                   zavg=zavg+z
                                   countz+=1
                                   pointstest.append([x,y,z,red,green,blue])
                if(countz!=0): 
                    points.append([xavg/countz,yavg/countz,zavg/countz,red,green,blue])        
