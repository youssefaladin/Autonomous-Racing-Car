...
class fastslam:
    ...
    def predict(self):
        ...
        if self.keys[0] == 1 and self.keys[1] == 1:
            ...
            for i in range(len(self.particles)):
                self.particles[i]=self.bicyclemodel(self.particles[i])
            self.update()
    ...
    def cones_association (self,cones):
                ...
                for cone in cones.markers:
                    ...
                    if (cone.pose.position.x<9) and (cone.pose.position.y<5 and cone.pose.position.y>-5): 
                    #finding position of cones with respect to the world
                                        ...
                                        if(f==0):
                                            ...
                                            if ((cone.color.r==0 and cone.color.g==0 and color.b==200)):
                                                    color="blue"
                                            else:
                                                    color="red"      
    ...
    def findclosest(self,oldcones):
            ...
            return zsensor        
    def bicyclemodel(self,particle):
         if (self.i==0):
              dt=0.05
              self.i=1
         else:
         ...
         particle.x=particle.x+self.vx*0.025
         particle.y=particle.y+self.vy*0.025
         particle.yaw=particle.yaw+self.angular_vel*0.025
...
if __name__ == '__main__':
    try:
        ...
        particlesrand = np.random.normal(loc=initial_pose, scale=[0.1, 0.1, np.radians(5)], size=(N_PARTICLE, 3))
        Particles=[]
