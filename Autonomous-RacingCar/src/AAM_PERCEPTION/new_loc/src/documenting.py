...
class callbacks_data:
  ...
  def cones_callback(self,cone_detections):
    ...
    for cone in cone_detections.markers:
      ...
      print("lm_x",cone.pose.position.x+1.5)
      print("lm_y",cone.pose.position.y)
...
def jacob_motion(x, call_backs_data):
    ...
    t=np.dot(Fx.T , jF)
    print("t",t,"t")
    G = np.eye(STATE_SIZE) + np.dot(t , Fx)
    print("jf",jF,"jf")
    print("G",G,"G")
    return G, Fx
...
def observation(Call_backs_data):
    ...
    print("Z after the observation function", z)
    return  z
...
def ekf_slam(xEst, PEst, z,call_backs_data):
    ...
    print(" PEst after prediction ", PEst)
    print("xEst after prediction", xEst)

    initP = np.eye(2)

    # Update
    xEst, PEst = update(xEst, PEst, z, initP)
    print(" PEst after update ", PEst)
    print("xEst after update", xEst)
...
def update(xEst, PEst, z, initP):
    ...
    for iz in range(len(z[:, 0])):  # for each observation
        ...
        print("k",K,"k") # Calculate Kalman Gain
        print("S_inversed",S,"S_inversed")
...
def main(call_backs_data):
    ...
    print("Initializtion of xEst:", xEst)
    PEst = np.eye(STATE_SIZE)
    print("initialization of PEst", PEst)
    # history
    hxEst = xEst
    x=0
    while x < 100:
